...
export interface Player {
  ...
  receivedExtraTime: boolean; // Track if player received extra time this round
}
...
export async function initDatabase() {
  ...
  try {
    ...
    // Check and add time bank columns to players table
    const timeBankColumns = [
      { name: 'time_bank', type: 'INTEGER DEFAULT 0' },
      { name: 'received_extra_time', type: 'BOOLEAN DEFAULT FALSE' }
    ];

    for (const column of timeBankColumns) {
      const columnCheck = await client.query(`
        SELECT column_name 
        FROM information_schema.columns 
        WHERE table_name = 'players' AND column_name = '${column.name}'
      `);
      
      if (columnCheck.rows.length === 0) {
        console.log(`➕ Adding ${column.name} column to players table...`);
        await client.query(`
          ALTER TABLE players 
          ADD COLUMN ${column.name} ${column.type}
        `);
  ...
  try {
    ...
    const players = playersResult.rows.map((p) => {
      return {
        ...
        timeBank: p.time_bank || 0,
        receivedExtraTime: p.received_extra_time || false,
      };
    });
  ...
  try {
    ...
    Object.entries(updates).forEach(([key, value]) => {
      ...
      if (key === "current_question") {
        ...
        let dbFieldName = key;
        if (key === "timeBank") {
          dbFieldName = "time_bank";
        } else if (key === "receivedExtraTime") {
          dbFieldName = "received_extra_time";
        }
...
export async function cleanupInactiveRooms(io?: SocketIOServer, inactivityThresholdMs: number = 600000): Promise<number> {
  const client = await pool.connect();
  try {
...
export async function cleanupOldRooms(io?: SocketIOServer): Promise<void> {
  const client = await pool.connect();
  try {
    // Delete rooms older than 4 hours or inactive for 4 hours
    const result = await client.query(`
      DELETE FROM rooms 
      WHERE created_at < NOW() - INTERVAL '4 hours'
         OR last_activity < NOW() - INTERVAL '4 hours'
      RETURNING id
    `);
