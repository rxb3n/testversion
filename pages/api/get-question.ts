...
function generateQuestion(sourceLanguage: "english" | "french" | "german" | "russian" | "japanese" | "spanish", targetLanguage: "english" | "french" | "german" | "russian" | "japanese" | "spanish"): Question {
  console.log(`🎯 API: Generating question from ${sourceLanguage} to ${targetLanguage}`)

  // Validate languages
  if (!["english", "french", "german", "russian", "japanese", "spanish"].includes(sourceLanguage) || 
      !["english", "french", "german", "russian", "japanese", "spanish"].includes(targetLanguage)) {
    console.error(`❌ API: Invalid language: ${sourceLanguage} or ${targetLanguage}`)
    throw new Error(`Invalid language: ${sourceLanguage} or ${targetLanguage}`)
  }
  ...
  // Get source word and correct answer
  const sourceWord = randomWord[sourceLanguage]
  const correctAnswer = randomWord[targetLanguage]
  
  if (!sourceWord) {
    console.error(`❌ API: No translation found for ${sourceLanguage} in word:`, randomWord)
    throw new Error(`No translation found for ${sourceLanguage}`)
  }
  
  if (!correctAnswer) {
    console.error(`❌ API: No translation found for ${targetLanguage} in word:`, randomWord)
    throw new Error(`No translation found for ${targetLanguage}`)
  }
  ...
  while (wrongAnswers.length < 3 && attempts < maxAttempts) {
    ...
    const wrongAnswer = randomWrongWord[targetLanguage]
  ...
  if (wrongAnswers.length < 3) {
    console.error(`❌ API: Could not generate enough wrong answers for ${targetLanguage}. Got ${wrongAnswers.length}/3`)
    throw new Error(`Could not generate enough wrong answers for ${targetLanguage}`)
  }
  ...
  const question: Question = {
    ...
    english: sourceWord, // This field name is misleading but kept for compatibility
    correctAnswer,
    options,
  }
...
export default function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    const question = generateQuestion(sourceLanguage, targetLanguage)
